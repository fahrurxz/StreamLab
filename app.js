...
app.get('/setup-account', async (req, res) => {
  try {
    ...
    // Allow access if no users exist (first-time setup)
    if (!usersExist) {
    ...
    // Allow access with force parameter to bypass restrictions
    ...
    // If user is logged in and tries to access setup, redirect to dashboard
    if (req.session.userId) {
      const user = await User.findById(req.session.userId);
    ...
    // If users exist but no session, allow setup (for creating additional accounts)
    ...
    res.render('setup-account', {
      title: 'Complete Your Account',
      user: {},
      error: 'An error occurred loading the setup page. Please try again.'
    });
  }
});
  ...
  try {
    ...
    if (!usersExist) {
      ...
      try {
        ...
        console.error('Additional user creation error:', error);
        return res.render('setup-account', {
          title: 'Create Account',
          user: {},
          error: 'Failed to create user. Please try again.'
...
app.listen(port, '0.0.0.0', async () => {
  ...
  console.log(`StreamLab running at:`);
